...
local error0 = error
function error(err)
	error0(debug.traceback(err))
end

local assert0 = assert
function assert(bool,err)
	assert0(bool,debug.traceback(err))
end
...
local function _dump(t,depth,lv,keysub)
	...
	if meta then
		if type(meta)=='table' and not dumped[(meta)] then
			dumped[(meta)] = true
...
--debug.findobj( debug.getregistry(), function(o)return o==? end, 'REG' ) 
...
function debug.profiler(maxsec) --统计性能,maxsec时间
	local t0 = clock()
	local t = {}
